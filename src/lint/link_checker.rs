...
pub async fn check_links_in_directory(content_dir: &Path) -> KrikResult<Vec<BrokenLink>> {
    debug!("Starting parallel link scanning in directory: {}", content_dir.display());
    ...
    let broken_links = stream::iter(links_to_check)
        .map(|link| {
            ...
            async move {
                check_single_link_with_logging(client, link).await
            }
        })
    ...
    info!("Link checking completed. {} working, {} broken, {} total", 
        working_links, broken_links.len(), total_links
    );
    Ok(broken_links)
}
...
fn collect_links_from_files(content_dir: &Path) -> KrikResult<Vec<LinkToCheck>> {
        ...
        .filter_map(|e| e.ok())
    {
        ...
        if file_link_count > 0 {
            debug!("Found {} HTTP(S) links in {}", file_link_count, path.display());
        }
    }
    
    debug!("Scanned {} files, found {} total HTTP(S) links", files_scanned, links_to_check.len());
    Ok(links_to_check)
}
...
async fn check_link(client: &Client, url_str: &str) -> Result<(), String> {
    ...
    if status.is_success() || status.is_redirection() {
        ...
        Err(format!("HTTP {}: {}", status.as_u16(), status.canonical_reason().unwrap_or("Unknown")))
    }
}